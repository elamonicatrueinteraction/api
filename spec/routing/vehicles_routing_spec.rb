...
RSpec.describe BankAccountsController, type: :routing do
  describe 'some REST resources' do
    ...
    it { expect(post: '/vehicles').to route_to( routes_params.merge(controller: 'vehicles', action: 'create') ) }
    it { expect(put: '/vehicles/1').to route_to( routes_params.merge(controller: 'vehicles', action: 'update', id: '1') ) }
  ...
  describe 'some resources under shippers' do
    ...
    it { expect(get: '/shippers/1/vehicles').to route_to( routes_params.merge(controller: 'vehicles', action: 'index') ) }
    ...
    it { expect(post: '/shippers/1/vehicles').to route_to( routes_params.merge(controller: 'vehicles', action: 'create') ) }
    it { expect(put: '/shippers/1/vehicles/1').to route_to( routes_params.merge(controller: 'vehicles', action: 'update', id: '1') ) }
