...
describe 'Cancel Remote Payment Job' do
  ...
  context 'when payment doesnt exist' do
    it 'returns not found' do
      get "/jobs/cancel_remote_payment/5?token=#{token}"
      expect(response).to have_http_status(:not_found)
    end
  end

  context 'when payment doesnt have remote' do
    let!(:payment) { create(:payment, :without_remote, payable: order) }
    it 'returns unprocessable entity' do
      get "/jobs/cancel_remote_payment/#{payment.id}?token=#{token}"
